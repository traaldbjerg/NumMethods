reset
set pm3d map
set title "Distribution de température dans la pièce numéro 23, solution par UMFPACK + two grid method (°C)"
set xlabel "x (m)"
set ylabel "y (m)"
set zlabel "T (°C)"
set xrange [0.0:5.50]
set yrange [0.0:5.50]
set zrange [-50.0:50.0]
...
do for [mode in "min"] { # pour des petits pas de discrétisation, permet que la fenêtre soit à la bonne couleur
    ...
    splot "mat/out_two_grid.dat" using 2:1:3 with pm3d
}
